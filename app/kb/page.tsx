...
export default function KbPage() {
  ...
  function handleFileUpload(file: File) {
    ...
    if (ext !== 'txt' && ext !== 'md' && ext !== 'markdown') {
      setSaveError('Aceita apenas .txt e .md. Para PDF, cole o conteúdo como texto.')
      return
    }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editingDoc) {
        ...
        savedId = data.id
        setDocs(prev => [{ ...payload, id: savedId, source_type: 'texto' } as KbDoc, ...prev])
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {(filterSearch || filterCat || filterVert) && (
          <button
            onClick={() => { setFilterSearch(''); setFilterCat(''); setFilterVert('') }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: '#E5E7EB' }}>
              <h2 className="text-base font-semibold text-gray-900">{editingDoc ? 'Editar documento' : 'Novo documento'}</h2>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors"
                    >
                      <Upload size={12} /> Importar
                    </button>
                  </div>
                </div>
                <input ref={fileInputRef} type="file" accept=".txt,.md,.markdown" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFileUpload(f); e.target.value = '' }} />
                {editorTab === 'write' ? (
                  <textarea
                    value={form.content}
                    onChange={(e) => setForm(p => ({ ...p, content: e.target.value }))}
                    placeholder="Escreva em Markdown…"
                    className="w-full h-56 px-3 py-2.5 border rounded-lg text-sm text-gray-800 font-mono focus:ring-2 focus:ring-indigo-200 outline-none resize-none"
                    style={{ borderColor: '#E5E7EB' }}
                  />
                ) : (
                  <div className="w-full h-56 overflow-y-auto px-3 py-2.5 border rounded-lg bg-gray-50" style={{ borderColor: '#E5E7EB' }}>
                    {form.content ? (
            ...
            <div className="flex items-center justify-end gap-2 px-6 py-4 border-t" style={{ borderColor: '#E5E7EB' }}>
              ...
              <button
                ...
                style={{ background: '#6366F1' }}
              >
                {saving ? <><Loader2 size={14} className="animate-spin" /> Salvando…</> : (editingDoc ? 'Salvar alterações' : 'Criar documento')}
              </button>
            </div>
          </div>
        </div>
