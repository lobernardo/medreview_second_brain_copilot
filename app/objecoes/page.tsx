...
function winRateBadge(rate: number | null) {
...
export default function ObjEcoesPage() {
  ...
  async function handleSave() {
    if (!form.topic.trim() || !form.recommended_response.trim()) { setSaveError('Tema e resposta recomendada são obrigatórios.'); return }
    setSaving(true); setSaveError('')
    try {
      const payload = {
        topic: form.topic.trim(), definition: form.definition || null, real_meaning: form.real_meaning || null,
        vertical: form.vertical || null, recommended_response: form.recommended_response,
      ...
      if (editingObj) {
        ...
        const { data, error } = await supabase.from('objection_patterns').insert({ ...payload, times_seen_total: 0 }).select('id').single()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Objeções</h1>
          <p className="text-sm text-gray-500 mt-0.5">{objs.length} objeção{objs.length !== 1 ? 'ões' : ''} mapeadas</p>
        </div>
        {isGestor && (
      ...
      <div className="flex flex-wrap gap-2">
        <div className="relative">
          ...
          <input type="text" value={filterSearch} onChange={(e) => setFilterSearch(e.target.value)} placeholder="Buscar por tema…" className="pl-9 pr-3 py-2 border rounded-lg text-sm text-gray-800 focus:ring-2 focus:ring-indigo-200 outline-none bg-white" style={{ borderColor: '#E5E7EB', minWidth: '180px' }} />
        </div>
      ...
      {!loading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          {isGestor && <button onClick={openCreate} className="mt-3 text-indigo-600 text-sm hover:underline">Adicionar primeira objeção</button>}
        </div>
      )}
      ...
      {!loading && (
        <div className="grid gap-4 lg:grid-cols-2">
          {filtered.map(obj => {
            ...
            return (
              <div key={obj.id} className="bg-white border rounded-xl p-5 space-y-3 hover:shadow-md transition-shadow" style={{ borderColor: '#E5E7EB', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                {/* Header */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-bold text-gray-900 leading-snug">{obj.topic}</h3>
                    {obj.real_meaning && <p className="text-[12px] text-gray-500 italic mt-0.5">&ldquo;{obj.real_meaning}&rdquo;</p>}
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    {wr && <span className="text-[11px] px-2.5 py-0.5 rounded-full font-semibold" style={{ color: wr.color, background: wr.bg }}>win {wr.label}</span>}
                    {obj.vertical && <VerticalBadge vertical={obj.vertical} />}
                    {isGestor && (
                      <>
                        <button onClick={() => openEdit(obj)} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"><Edit2 size={13} /></button>
                        {confirmDeleteId === obj.id ? (
                          <div className="flex items-center gap-1">
                            <button onClick={() => handleDelete(obj.id)} className="px-2 py-0.5 text-[11px] text-white rounded font-medium" style={{ background: '#EF4444' }}>Sim</button>
                            <button onClick={() => setConfirmDeleteId(null)} className="p-1 text-gray-400"><X size={12} /></button>
                          </div>
                        ) : (
                          <button onClick={() => setConfirmDeleteId(obj.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={13} /></button>
                        )}
                      </>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] flex flex-col shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5 col-span-2">
                  ...
                  <input type="text" value={form.topic} onChange={(e) => setForm(p => ({ ...p, topic: e.target.value }))} placeholder="Ex: Preço muito alto" className="w-full px-3 py-2 border rounded-lg text-sm text-gray-800 focus:ring-2 focus:ring-indigo-200 outline-none" style={{ borderColor: '#E5E7EB' }} />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="number" min="0" max="100" value={form.win_rate} onChange={(e) => setForm(p => ({ ...p, win_rate: e.target.value }))} placeholder="Ex: 65" className="w-full px-3 py-2 border rounded-lg text-sm text-gray-800 focus:ring-2 focus:ring-indigo-200 outline-none" style={{ borderColor: '#E5E7EB' }} />
                </div>
                <div className="space-y-1.5 col-span-2">
                  <label className="text-xs font-semibold uppercase tracking-wide text-gray-500">Definição</label>
                  <input type="text" value={form.definition} onChange={(e) => setForm(p => ({ ...p, definition: e.target.value }))} placeholder="Breve definição da objeção…" className="w-full px-3 py-2 border rounded-lg text-sm text-gray-800 focus:ring-2 focus:ring-indigo-200 outline-none" style={{ borderColor: '#E5E7EB' }} />
                </div>
                <div className="space-y-1.5 col-span-2">
                  <label className="text-xs font-semibold uppercase tracking-wide text-gray-500">O que o lead realmente quer dizer</label>
                  <input type="text" value={form.real_meaning} onChange={(e) => setForm(p => ({ ...p, real_meaning: e.target.value }))} placeholder="Ex: Ainda não vejo valor suficiente…" className="w-full px-3 py-2 border rounded-lg text-sm text-gray-800 focus:ring-2 focus:ring-indigo-200 outline-none" style={{ borderColor: '#E5E7EB' }} />
                </div>
                <div className="space-y-1.5 col-span-2">
                  <label className="text-xs font-semibold uppercase tracking-wide text-gray-500">Resposta recomendada *</label>
                  <textarea value={form.recommended_response} onChange={(e) => setForm(p => ({ ...p, recommended_response: e.target.value }))} placeholder="Como o closer deve responder…" rows={4} className="w-full px-3 py-2.5 border rounded-lg text-sm text-gray-800 focus:ring-2 focus:ring-indigo-200 outline-none resize-none" style={{ borderColor: '#E5E7EB' }} />
                </div>
                <div className="space-y-1.5 col-span-2">
                  <label className="text-xs font-semibold uppercase tracking-wide text-gray-500">O que NÃO dizer</label>
                  <textarea value={form.what_not_to_say} onChange={(e) => setForm(p => ({ ...p, what_not_to_say: e.target.value }))} placeholder="Frases e abordagens que pioram a situação…" rows={3} className="w-full px-3 py-2.5 border rounded-lg text-sm text-gray-800 focus:ring-2 focus:ring-indigo-200 outline-none resize-none" style={{ borderColor: '#E5E7EB' }} />
                </div>
                <div className="space-y-1.5 col-span-2">
                  <label className="text-xs font-semibold uppercase tracking-wide text-gray-500">Proof points</label>
                  <textarea value={form.proof_points} onChange={(e) => setForm(p => ({ ...p, proof_points: e.target.value }))} placeholder="Dados, cases e evidências que comprovam o valor…" rows={3} className="w-full px-3 py-2.5 border rounded-lg text-sm text-gray-800 focus:ring-2 focus:ring-indigo-200 outline-none resize-none" style={{ borderColor: '#E5E7EB' }} />
                </div>
              </div>
            </div>
            <div className="flex items-center justify-end gap-2 px-6 py-4 border-t" style={{ borderColor: '#E5E7EB' }}>
              ...
              <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-white disabled:opacity-60" style={{ background: '#6366F1' }}>
                {saving ? <><Loader2 size={14} className="animate-spin" /> Salvando…</> : (editingObj ? 'Salvar' : 'Criar objeção')}
              </button>
            </div>
          </div>
        </div>
